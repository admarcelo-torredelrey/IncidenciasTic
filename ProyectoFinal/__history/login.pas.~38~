...
begin
    ...
    //Si existe prueba la contraseña, si no indica un error
    if  locateSuccess then
    begin
      //si la contraseña es correcta entra, si no indica un error
...
//Checkbox para dejar visible o invisible la contraseña
procedure Txlogin.chbxPasswordClick(Sender: TObject);
begin

      // si está marcado, lo volveremos visible
      if chbxPassword.Checked then
      begin
        txtPassword.PasswordChar:= #0;
      end
      else
      //si no lo está, en invisible.
      begin
